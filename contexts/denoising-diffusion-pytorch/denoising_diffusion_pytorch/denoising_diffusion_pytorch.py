...
class GaussianDiffusion(nn.Module):
    ...
    def interpolate(self, x1, x2, t = None, lam = 0.5):
        ...
        x1_min = x1.min()
        x1_max = x1.max()
        x1 = 2 * (x1 - x1_min) / (x1_max - x1_min) - 1

        x2_min = x2.min()
        x2_max = x2.max()
        x2 = 2 * (x2 - x2_min) / (x2_max - x2_min) - 1
...
class Dataset(Dataset):
    def __init__(
        ...
        if str(self.paths[0])[-4:] == '.txt':
            self.elevations = [data - np.min(data) for data in [np.loadtxt(file, delimiter=',') for file in self.paths]]
            self.elevations = np.array(self.elevations)
            self.elevations /= 5.0 ##np.max(self.elevations)
            self.elevations = [self.transform(Image.fromarray(img, mode='L')) for img in self.elevations]
